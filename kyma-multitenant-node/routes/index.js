...
/* Variables and constants to help define logic for APIRule creation */
 ...
 router.put('/callback/v1.0/tenants/*', async function(req, res) {
    ...
    try {
        ...
        if (result.response.statusCode == 200) {
            console.log(apiRuleTempl.metadata.name + ' já existe.');
            res.status(200).send(tenantAppURL);
        }
    } catch (err) {
        //create apirule if non-exist
        console.warn(apiRuleTempl.metadata.name + ' não existe, criando...');
        try {
            ...
            if (createResult.response.statusCode == 201) {
                console.log("Regra API criada!");
                res.status(200).send(tenantAppURL);
            }
        } catch (err) {
            console.log(err);
            console.error("Falha ao criar APIRule");
            res.status(500).send("Erro ao criar APIRule");
        }
    }
    console.log("saindo do onboarding...");
    res.status(200).send(tenantAppURL)
});
...
router.delete('/callback/v1.0/tenants/*', async function(req, res) {
    ...
    try {
        const result = await k8sApi.deleteNamespacedCustomObject(
            ...
            apiRuleTempl.metadata.name);
        if (result.response.statusCode == 200) {
            console.log("Regra API deletada!");
        }
    } catch (err) {
        console.error(err);
        console.error("Erro ao deletar API Rule");
    }

    res.status(200).send("deletada");
});
//************************************************************************************************
